...
def make_answer_response(vm, answers):
    ...
    try:
        ...
        raise ValueError("Task Error {}".format(err))
...
def answer_question(vm, responses):
    ...
    for response in responses:
        try:
            ...
            raise AttributeError("answer failed: %s" % e)
    ...
    while True:
        if check_answer_question_status(vm):
            if answers:
                ...
                raise AttributeError("%s" % (vm.runtime.question.text))
        ...
        if task.info.state == vim.TaskInfo.State.error:
            ...
            try:
                ...
                ValueError(error_msg, host_thumbprint)
